<?php
	class Frontpage {
		public static function getPosts() {
			...
			$query = "SELECT p.post_id, p.group_id, p.user_id, p.file_id, p.event_id, p.text, p.date, u.name, c.count, g.name
			FROM posts p
			...
			$ok = $stmt->bind_result($post_id, $group_id, $user_id, $file_id, $event_id, $text, $date, $name, $count, $group_name);
			if(false === $ok)
